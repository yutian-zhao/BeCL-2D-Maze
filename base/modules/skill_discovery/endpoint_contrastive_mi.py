...
class EndpointDiscriminator(Discriminator):
    def __init__(self, *args, mode=None, use_reg=False, **kwargs):
        ...
        self.input_normalizer = Normalizer(self.state_size * 3) if self.normalize_inputs else nn.Sequential()
        self.layers = create_nn(
            input_size=self.state_size * 3,
            output_size=self.n,
    ...
    #     similarity_matrix.masked_fill_(torch.logical_or(~t_mask, labels)&(~candidate_mask), float("-inf"))
